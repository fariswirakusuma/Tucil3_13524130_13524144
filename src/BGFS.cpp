...
BGFS_Solver::BGFS_Solver(MapData* m, HeuristicType hType) : PathFinder(m), heuristic(hType) {
    double h_val = calculate_h(start, 0);
    openSet.push(Node(start, 0, 0.0, h_val, h_val));
    actualG[{start.first, start.second, 0}] = 0.0;
}

double BGFS_Solver::calculate_h(cord current, int currentSeq) {
...
void BGFS_Solver::expandNode(Node current) {
...
bool BGFS_Solver::isFinished() { return found || openSet.empty(); }
vector<cord> BGFS_Solver::getOpenSetCoords() const {
    vector<cord> res;
    for(auto const& n : openSet.get_container()) res.push_back(n.c);
    return res;
}
vector<cord> BGFS_Solver::getClosedSetCoords() const {
    set<cord> u; for(auto const& s : closedSet) u.insert({s.r, s.c});
    return vector<cord>(u.begin(), u.end());
}
vector<cord> BGFS_Solver::getFinalPathCoords() const {
